...
namespace nfx::core::benchmark
{
	...
	{
		const uint32_t initialHash = nfx::core::hashing::constants::DEFAULT_FNV_OFFSET_BASIS;
		const uint8_t testByte = 'A';

		for ( auto _ : state )
		{
			uint32_t hash = nfx::core::hashing::fnv1a<nfx::core::hashing::constants::DEFAULT_FNV_PRIME>( initialHash, testByte );
	...
	static void BM_ManualFNV1a_Short( ::benchmark::State& state )
	{
		for ( auto _ : state )
		{
			...
			for ( const auto& str : shortStrings )
			{
				uint32_t hash = nfx::core::hashing::constants::DEFAULT_FNV_OFFSET_BASIS;
				for ( char c : str )
				{
					hash = nfx::core::hashing::fnv1a<nfx::core::hashing::constants::DEFAULT_FNV_PRIME>( hash, static_cast<uint8_t>( c ) );
				}
				totalHash += hash;
			}
			::benchmark::DoNotOptimize( totalHash );
